...
async function joinProject(projectId, userId) {
  await pool.query(
    'INSERT INTO volunteers (project_id, user_id) VALUES ($1, $2)',
    [projectId, userId]
  );
}
...
async function leaveProject(projectId, userId) {
  await pool.query(
    'DELETE FROM volunteers WHERE project_id = $1 AND user_id = $2',
    [projectId, userId]
  );
}
...
async function isUserInProject(projectId, userId) {
  const res = await pool.query(
    'SELECT 1 FROM volunteers WHERE project_id = $1 AND user_id = $2',
    [projectId, userId]
  );
...
async function countVolunteers(projectId) {
  const res = await pool.query(
    'SELECT COUNT(*) AS total FROM volunteers WHERE project_id = $1',
    [projectId]
  );
...
async function updateVolunteerCount(projectId) {
  ...
  await pool.query(
    'UPDATE projects SET current_volunteer = $1 WHERE id = $2',
    [total, projectId]
  );
}
...
async function removeUserFromAllActiveProjects(userId) {
  ...
  await pool.query(`
    ...
    WHERE user_id = $1
    AND project_id IN (SELECT id FROM projects WHERE status = 'ativo')
  `, [userId]);
