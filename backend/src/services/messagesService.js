...
async function saveMessage(projectId, senderId, content) {
  const q = `
    INSERT INTO prata.messages (project_id, sender_id, content)
    VALUES ($1, $2, $3)
  ...
  const userRes = await pool.query('SELECT name FROM prata.users WHERE id = $1', [senderId]);
...
async function getMessagesByProject(projectId, limit = 100) {
  const q = `
    ...
    FROM prata.messages m
    LEFT JOIN prata.users u ON m.sender_id = u.id
